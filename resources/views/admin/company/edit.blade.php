                ...
                <form method="POST" action="{{ route('admin.company.update', $company) }}" enctype="multipart/form-data"> 
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" 
                          required autofocus autocomplete="name" value="{{$company->name}}"/>
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="logo" :value="__('logo')" />
                        <img src="{{ asset('storage/logos/' . basename($company->logo)) }}" alt="" class="rounded-2xl object-cover w-[90px] h-[90px]">
